...
function execute(id,caster,target,log=[]){const card=lookup(id);if(!card||id==='faith')return log;let atk=caster.atkBuff;const def=caster.defBuff;caster.atkBuff=0;caster.defBuff=0;const previous=caster.lastCard;caster.lastCard=id;
 ...
 if(previous==='study'&&id==='research'){caster.research++;log.push('组合·求证：研究额外积累 1 次。');}
...
function create(floor,type,pHand,eHand,enemyHp,route='standard'){route=type==='dice'?'standard':strategy.routes[route]?route:'standard';const routeRules=strategy.routes[route];const s={version:1,floor,type,route,turn:1,seed:(Date.now()^Math.floor(Math.random()*4294967296))>>>0,winner:null,playerWins:0,enemyWins:0};const valid=h=>Array.isArray(h)&&h.length===3&&new Set(h).size===3&&h.every(id=>lookup(id));s.player=entity('旅人',10,valid(pHand)?pHand:deck(s));s.enemy=entity('无序者',(enemyHp||10+Math.min(10,(floor-1)*.12))*routeRules.hpMultiplier,valid(eHand)?eHand:deck(s));for(const actor of [s.player,s.enemy]){actor.damageBonus=route==='silence'?1:0;actor.healingBlocked=route==='silence';}s.intent=type==='card'?intent(s):null;return s;}
function restore(raw,floor){if(!raw||raw.version!==1||raw.floor!==floor||!['card','dice'].includes(raw.type)||raw.winner!==null||!Number.isInteger(raw.seed)||raw.seed<0||raw.seed>4294967295||!Number.isInteger(raw.turn)||raw.turn<1)return null;
 ...
 if(raw.intent!==null&&(!lookup(raw.intent)||!raw.enemy.hand.includes(raw.intent)))return null;const restored=JSON.parse(JSON.stringify(raw));restored.player.name='旅人';restored.enemy.name='无序者';restored.route=strategy.routes[raw.route]?raw.route:'standard';for(const actor of [restored.player,restored.enemy]){actor.lastCard=lookup(actor.lastCard)?actor.lastCard:null;actor.damageBonus=restored.route==='silence'?1:0;actor.healingBlocked=restored.route==='silence';}return restored;
}
